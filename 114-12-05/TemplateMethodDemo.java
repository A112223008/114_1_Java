...
abstract class Character {
    ...
    health -= damage;
    System.out.println(glyph("💥", "[HIT]") + " " + name + " 受到 " + damage + " 點傷害！剩餘HP: " + health);
    }
    ...
    public final void performBattle(Character opponent) {
        System.out.println("\n╔════════════════════════════════════╗");
    System.out.println("  " + glyph("⚔️","[VS]") + "  " + name + " vs " + opponent.getName());
        System.out.println("╚════════════════════════════════════╝");
        
        // 步驟 1：戰前檢查（具體方法）
        if (!preBattleCheck(opponent)) {
            System.out.println(glyph("❌","[X]") + " 戰鬥無法進行！\n");
            return;
        }
    ...
    // Helper: try to print emoji when available, otherwise use ASCII fallback.
    // We detect UTF-8 in the default charset as a heuristic.
    private static String glyph(String emoji, String asciiFallback) {
        try {
