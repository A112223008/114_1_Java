...
public class Account {
    ...
    public Account(String accountNumber, String ownerName, String ownerID, double initialBalance) {
        ...
        // 記錄開戶日期與時間
        this.openDate = new Date(now.getMonthValue(), now.getDayOfMonth(), now.getYear());
        this.openTime = new Time2(now.getHour(), now.getMinute(), now.getSecond());// 假設 Date 類別有無參數建構子，會設定為當前日期
    }
    ...
    public String toString() {
        return String.format("帳戶號碼: %s, 持有人: %s, 餘額: %.2f, 開戶日期: %s, 開戶時間: %s",
                accountNumber, owner.toString(), balance, openDate.toString(), openTime.toString());
    }
}
